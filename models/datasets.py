...
class StanfordDogs(torchvision.datasets.VisionDataset):
    ...
    def __init__(
        ...
        self.targets = tuple(
            self.get_class_index_from_image_filename(image_filename) for image_filename in self.image_filenames)
    ...
    def read_unique_class_names(self):
        ...
        image_filenames = self.read_file_list_from_mat(
            filename=os.path.join(self.root, "file_list.mat"))
        class_names = set()
        for image_filename in image_filenames:
            class_name = self.get_class_name_from_image_filename(
    ...
    def read_image_filenames(self):
        ...
        image_filenames = tuple(os.path.join(
            self.root, "Images", image_filename) for image_filename in image_filenames)
    ...
    def download(self):
        ...
        for source in self.sources:
            ...
            torchvision.datasets.utils.download_url(
                url=source["url"], root=self.root, filename=source["filename"], md5=source["md5"])
            ...
            if not all(os.path.exists(os.path.join(self.root, extracted_filename)) for extracted_filename in source["extracted_filenames"]):
                print("Extracting '{}' to '{}'".format(
                    source["filename"], self.root))
                with tarfile.open(full_filename, "r") as tar:
                    tar.extractall(path=self.root)
            else:
                print("File '{}' was already extracted, skipped extraction".format(
                    source["filename"]))
